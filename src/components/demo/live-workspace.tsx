...
import { BubbleSurfacePanel } from "../bubblesurface";
...
export function LiveWorkspace({model}:{model:LiveWorkspaceModel}) {
  ...
  useEffect(()=>{let disposed=false,integration:Awaited<ReturnType<typeof initializeLiveWebMcp>>|null=null;
    void initializeLiveWebMcp({onChange:state=>{if(!disposed)setSurface({status:state.available?"ACTIVE":"UNAVAILABLE",tools:state.registered})},
      onError:()=>{if(!disposed)setSurface(value=>({...value,status:"ERROR"}))}}).then(value=>{if(disposed)void value.dispose();else integration=value});
  ...
  return <main className={styles.page} data-responsive-shell="true">
      ...
      <div className={styles.content}>
        ...
        <div className={styles.mainGrid}>
          ...
          <aside className={styles.sideColumn}>
            ...
            <section className={styles.card}><CardHeader title="Workflow activity" meta="Authoritative events"/>{activity.length?<ol className={styles.toolActivity}>{activity.slice(-6).reverse().map(event=><li key={event.id}><span className={event.status==="FAILED"?styles.failed:styles.activityDot}/><div><strong>{event.toolName} {event.status==="STARTED"?"invoked":event.status.toLowerCase()}</strong><small>{event.actorId?`Agent · ${event.actorId}`:"WebMCP source"} · {formatTime(event.occurredAt)} UTC</small></div></li>)}</ol>:<div className={styles.ready}><span><Icon name="check" size={18}/></span><div><strong>Workspace ready</strong><p>No agent or human workflow activity has been recorded yet.</p></div></div>}</section>
          </aside>
        </div>
    ...
    <button className={styles.surfaceTab} aria-expanded={surfaceOpen} aria-controls="bubblesurface-drawer" onClick={()=>setSurfaceOpen(open=>!open)}><span className={styles.surfaceMark}><Icon name="surface" size={22}/></span><span className={styles.tabText}><strong>BubbleSurface</strong><small>Idle · no action required</small></span><span>{surfaceOpen?"×":"←"}</span></button>
    <aside id="bubblesurface-drawer" className={`${styles.drawer} ${surfaceOpen?styles.drawerOpen:""}`} aria-hidden={!surfaceOpen}><div className={styles.drawerTitle}><span>Embedded control surface</span><button onClick={()=>setSurfaceOpen(false)} aria-label="Close BubbleSurface">×</button></div><BubbleSurfacePanel model={model.humanSurface} mode="embedded" className={styles.humanPanel}/></aside>
...
type ToolActivity={id:string;eventType:string;toolName:string;status:"STARTED"|"SUCCEEDED"|"FAILED";actorType:string;actorId:string|null;occurredAt:string;lifecycleVersion:number|null};
