...
import { demoReviewPrincipalResolver } from "../webmcp/demo-principal-resolver";

const id=z.string().trim().min(1).max(100);
const review=z.object({proposalVersion:z.number().int().positive(),expectedLifecycleVersion:z.number().int().positive(),actorId:id.optional(),comment:z.string().max(1000).optional()}).strict();
...
export const reviewAction=async(actionId:string,kind:"approve"|"reject"|"modify",request:Request)=>{try{const raw=await request.json();const input=(kind==="modify"?modify:review).parse(raw);const principal=await demoReviewPrincipalResolver.resolve(request);const trusted={...input,actorId:principal.id};return NextResponse.json(kind==="approve"?proposalReviewService.approve({actionId,...trusted}):kind==="reject"?proposalReviewService.reject({actionId,...trusted}):proposalReviewService.modify({actionId,...trusted}));}catch(e){return error(e);}};
