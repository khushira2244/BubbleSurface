...
export class CapabilityRefreshService {
  ...
  constructor(private readonly contexts: CapabilityContextService,
    ...
    private readonly audit: WebMcpAuditRecorder) { this.reconciler = new BrowserCapabilityReconciler(browser); }
  ...
  async refreshCapabilities(subjectType: "INCIDENT" | "FINDING", subjectId: string) {
    ...
    const registrations: BrowserToolRegistration[] = desired.map((toolName) => {
    ...
    const reconciled = await this.reconciler.reconcile(registrations);
    for (const name of reconciled.appliedRemoved) {
      const toolName = name as WebMcpToolName, tool = this.tools[toolName];
    ...
    for (const name of reconciled.appliedAdded) {
      const toolName = name as WebMcpToolName, tool = this.tools[toolName];
